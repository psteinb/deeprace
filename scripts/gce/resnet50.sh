...
do
	 ...
	 do
         if [[ -e /home/steinbac/resnet56v1-short-gce-bs${i}-singularity_${r}.tsv ]];then
             echo "$0 batch_size=$i run ${r}/10 already exists"
             continue;
         else
	         echo "$0 batch_size=$i run ${r}/10"
             singularity exec -B /home/steinbac/deeprace:/deeprace --nv /home/steinbac/tf1.3-plus.simg python /deeprace/deeprace.py train -O batch_size=${i} -c "k80:1,fs:gce,singularity:gce" -t /home/steinbac/resnet56v1-short-gce-bs${i}-singularity_${r}.tsv -e 10 resnet56v1
         fi
	 done
done
